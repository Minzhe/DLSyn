...
with open(data_path, 'rb') as f:
    data = pkl.load(f)
X_train, X_test, y_train, y_test = data['X_train'], data['X_test'], data['y_train'].iloc[:,0], data['y_test'].iloc[:,0]
# x = 100000
...
model = neural_net.fcnn(input_length=X_train.shape[1], loss='mse', hidden_layer_sizes=(256,128,128), output_activation='linear', output_length=1, dropout=0.2, learning_rate=3e-4)
model.train(X_train, y_train, model_name='test-all@', model_path=model_path, validation_data=(X_test, y_test), epochs=200, tolerance=30)
