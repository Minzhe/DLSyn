...
##############################    function   ################################
def read_mutation(data_path, cell_path, mut_path, minMut=1, filter='all_gene', impute=False):
    ...
    # outpath
    out_path = os.path.join(proj_dir, 'data/Curated/ALMANAC/Genomics/curated.mutation.min_{}.{}.csv'.format(minMut, filter))
    mut.to_csv(out_path)
...
read_mutation(data_path, cell_path, mut_path, minMut=1, filter='cancer_gene', impute=True)